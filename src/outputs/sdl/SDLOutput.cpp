...
namespace outputs {
namespace sdl {
bool NRGlyphComparator::operator()(const nr_glyph &a, const nr_glyph &b) const {
...
void SDLOutput::pollEvents() {
    ...
    while (SDL_PollEvent(&event)) {
        ...
        if (event.type == SDL_MOUSEBUTTONDOWN) {
            int tx = (event.button.x - dx) / fontWidth;
            int ty = (event.button.y - dy) / fontHeight;
            fingerMap.emplace(0, Vec2Int(tx, ty));
            outputEvent.type = webrogue_event_type::Click;
        ...
        if (event.type == SDL_FINGERDOWN) {
            ...
            int tx = (event.tfinger.x * windowWidth - dx) / fontWidth;
            int ty = (event.tfinger.y * windowHeight - dy) / fontHeight;
            fingerMap.emplace(0, Vec2Int(tx, ty));
            fingerMap.emplace(event.tfinger.fingerId, Vec2Int(tx, ty));
            outputEvent.type = webrogue_event_type::Click;
            outputEvent.data1 = tx;
            outputEvent.data2 = ty;
            lastClick = std::chrono::steady_clock::now();
    ...
    if ((std::chrono::steady_clock::now() - lastClick).count() > 200e6) {
        for (auto i = fingerMap.begin(); i != fingerMap.end(); i++) {
            outputEvent.type = webrogue_event_type::Click;
            outputEvent.data1 = i->second.x;
            outputEvent.data2 = i->second.y;
            lastClick = std::chrono::steady_clock::now();
            events.push(outputEvent);
        }
    }
...
void SDLOutput::drawInputToTexture() {
    ...
    for (int y = 0; y < storedSize.y; y++) {
        for (int x = 0; x < storedSize.x; x++) {
            nr_glyph ch = renderBuffer.at(x, y);
            nr_glyph lastCh = lastInput.at(x, y);
            if (!lastInputValid || NRGlyphComparator()(ch, lastCh) ||
                NRGlyphComparator()(lastCh, ch) || true) {
                lastInput.at(x, y) = ch;
                SDL_Texture *texture;
...
void SDLOutput::fpsSync() {
    uint32_t passed = SDL_GetTicks() - lastUpdateTick;
